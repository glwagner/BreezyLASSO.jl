...
for (run, label) in zip(runs, labels)
    ...
    ax1 = Axis(fig[1, 1:2], xlabel="x (km)", ylabel="z (m)", title=@sprintf("qᶜˡ (g kg⁻¹) at y = Ly/2, t = %.1f h", qxz.times[n] / 3600))
    hm1 = heatmap!(ax1, qxz[n]; colormap=:Blues)
    ylims!(ax1, 0, 2500)
    Colorbar(fig[1, 3], hm1)
    ax2 = Axis(fig[2, 1], xlabel="x (km)", ylabel="y (km)", title="w at 900 m (m s⁻¹)", aspect=1)
    wl = maximum(abs, wxy[n]) / 2
    hm2 = heatmap!(ax2, wxy[n]; colormap=:balance, colorrange=(-wl, wl))
    Colorbar(fig[2, 2], hm2)
    ax3 = Axis(fig[2, 3], xlabel="x (km)", ylabel="y (km)", title="LWP (g m⁻²)", aspect=1)
